# list of Brainfuck-targetting compilers and assemblers

this file is revised by Gemini.  
some categorization are still incorrect.  
and some column-selection are wrong.  
Generally, disassemblers can only be written for Class 0 and 1 if the expected input is not their own output.  

## [asm class 0](./asm_list_0.md)

No simulation required.  

## [asm class 0s](./asm_list_0s.md)

No simulation required, but these cannot represent certain Brainfuck programs.

## [asm class 1](./asm_list_1.md)

Simulation required.  

## [asm class 1s](./asm_list_1s.md)

Simulation required, and these cannot represent certain Brainfuck programs.

## [preprocessor](./pp_list.md)

## [high-level language](./hll_list.md)

## [high-level language (goto simulator)](./hll_list_g.md)

## [code-generation library](./lib_list.md)
